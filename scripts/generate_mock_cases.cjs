...
const generateCasesText = () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    cases.push({
      ...
      tags: selectedTags,
      type: 'case',
      caseStatus: statusOptions[Math.floor(Math.random() * statusOptions.length)],
      visibility: 'public',
      ownerId: 'seeder-bot',
      ownerName: 'Dr. Auto-Seeder',
  ...
  return `import { Asset } from '../types';\n\nexport const MOCK_CASES: Asset[] = ${JSON.stringify(cases, null, 2)};`;
}

fs.writeFileSync('./mockCases.ts', generateCasesText());
console.log('mockCases.ts gerado com sucesso!');
