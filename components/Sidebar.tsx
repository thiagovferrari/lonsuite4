...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, trashCount = 0 }) => {
  const NavItem = ({
    ...
    return (
      <div className="relative group flex min-w-0 flex-col items-center gap-1">
        <button
          onClick={() => setView(view)}
          aria-label={label}
          className={`relative flex h-[38px] w-[38px] items-center justify-center rounded-[12px] transition-all duration-300 md:h-[40px] md:w-[40px] ${isActive
        ...
        >
          <Icon size={19} strokeWidth={isActive ? 1.5 : 1} />
          {badge != null && badge > 0 && (
        ...
        {/* Tooltip */}
        <div className="absolute left-full ml-3 hidden px-2.5 py-1.5 bg-[#1d1d1f] text-white text-[10px] font-medium tracking-wider uppercase rounded-[8px] opacity-0 group-hover:opacity-100 transition-all whitespace-nowrap pointer-events-none z-[400] shadow-apple-md translate-x-1 group-hover:translate-x-0 md:block">
          {label}
